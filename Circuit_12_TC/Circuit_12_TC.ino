...
SHIFT REGISTER
  ...
  ***TC note: CANNOT figure out why I can't get the lights to all turn off 
  between steps on the marquee() function by sending a LOW signal to the 
  SRCLR pin on the register. I have a 10K resistor connected to that pin
  as well, so it should ground as soon as the output voltage from the 
  pin on the Arduino shifts from HIGH to LOW. If I disconnect the SRCLR pin
  from the Arduino pin, the lights do shut off in the next step (and then
  obviously don't come back on). (NB: it only does this shutting off thing
  when the resistor is connected to ground.) I also connected the SRCLR pin
  to an analog input so I could measure the voltage at each step, and it
  does go from near 1023 to 0, as expected -- and slowly decreases in voltage
  with each step when the digital output is disconnected and there's no 
  grounded resistor, also as expected. V confusing.*** 
...
{
  int index;
  int delayTime = 2000; // Time (milliseconds) to pause between LEDs
                       // Make this smaller for faster switching
  int clrpinVoltage;                  
  ...
  {
    digitalWrite(clrpin, HIGH); // TC: Set serial clear to high
    clrpinVoltage = analogRead(0);
    Serial.println(clrpinVoltage);
    shiftWrite(index, HIGH);    // Turn a LED on
    shiftWrite(index+4, HIGH);  // Skip four, and turn that LED on
    delay(delayTime);		// Pause to slow down the sequence
    digitalWrite(clrpin, LOW); // TC: Set serial clear to low to clear register
    clrpinVoltage = analogRead(0);
    Serial.println(clrpinVoltage);
    delay(delayTime);
